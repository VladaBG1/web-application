...
class NewsController extends Controller
{
    ...
    {
        if (!User::isAdmin()) {
            throw new \Exception('You have not access rights!');
        }
        $newsDb = new News();
        $data = $_POST['News'] ?? [];
    ...
    public function actionDeleteComment()
    {
        ...
        if (!Comment::canDelete($comment['comment_user_id'])) {
            throw new \Exception('You have no right to delete a comment!');
        }        
    ...
    {
        if (!User::isAdmin()) {
            throw new \Exception('Only the admin has right to delete posts!');
        }
        $newsId = $_GET['newsId'];
        $newsDb = new News();        
